 ...
 * @Description: gen
 * @Author: Mankeung
 * @Date: 2020-10-10 20:22:18
 * @LastEditors: Mankeung
 * @LastEditTime: 2020-10-11 17:48:00
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    metalsmith.use((files, metalsmith, done) => {
      ...
      Object.keys(files).forEach(fileName => {
        ...
        if (fileName === "package.json") files[fileName].contents = new Buffer.from(package_temp_content)
        // else
      ...
      remove(src)
      fs.unlinkSync(packjsonTemp.replace(/(\.download-temp[\\|\/])/g, ''))
